...
import React, { useState } from 'react';
import { Mail, ShieldCheck, ArrowLeft, UserCheck, RefreshCw } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ onSuccess }) => {
  ...
  // Step 3: Verify OTP & Log In Directly
  const handleVerifyOtp = (e: React.FormEvent) => {
    e.preventDefault();
    const enteredOtp = otp.join('');
    if (enteredOtp.length < 6) {
  ...
  return (
    <div className="min-h-screen bg-[#523519] text-[#2c1a0e] flex flex-col items-center justify-center p-4 font-sans selection:bg-[#523519] selection:text-white">
      ...
      <div className="w-full max-w-[430px] bg-[#ebdccb] rounded-2xl p-7 shadow-2xl relative border border-[#dfcaa6]/40">
        ...
        {/* Secure Badge */}
        <div className="w-full py-2 px-3 bg-[#ded0bd] border border-[#cfbeaa] rounded-lg text-xs font-semibold text-[#544131] flex items-center gap-2 justify-start mb-4">
          <UserCheck className="w-4 h-4 text-[#544131]" />
          <span>User Authentication Enabled</span>
        </div>
        ...
        {step === 'EMAIL' && (
          <form onSubmit={handleSendOtp} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 bg-[#3c2311] hover:bg-[#2b190c] text-[#f5ede4] font-semibold rounded-lg text-sm transition duration-150 flex items-center justify-center gap-2 shadow-md mt-2 disabled:opacity-60"
            >
            ...
            <div className="pt-4 text-center text-[11px] leading-relaxed text-[#7c6a59] border-t border-[#dfcaa6]/50 mt-4">
              <p>Welcome to Casa Terminal - Construction Marketplace.</p>
              <p className="mt-0.5 font-medium">Demo: Enter email, then any 6-digit OTP</p>
            </div>
          </form>
        )}
        ...
        {step === 'OTP' && (
          <form onSubmit={handleVerifyOtp} className="space-y-4 text-center">
            ...
            <p className="text-xs text-[#6e5d4f]">
              ...
              <span className="font-semibold text-[#3c2311] truncate max-w-full inline-block mt-0.5">
                {email}
              </span>
            </p>
            ...
            <button
              ...
              className="w-full py-3 bg-[#3c2311] hover:bg-[#2b190c] text-[#f5ede4] font-semibold rounded-lg text-sm transition duration-150 shadow-md disabled:opacity-60"
            >
              {isLoading ? 'Verifying...' : 'Verify & Log In'}
            </button>

            <div className="flex items-center justify-between text-xs pt-2 font-medium text-[#544131]">
              <button
                type="button"
                onClick={() => {
                  setStep('EMAIL');
